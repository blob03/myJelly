...
const AlphaPickerContainer: FC<AlphaPickerContainerProps> = ({ viewQuerySettings, setViewQuerySettings }) => {
    ...
    const onAlphaPickerChange = useCallback((e) => {
        ...
        if (newValue === '#') {
            updatedValue = {NameLessThan: 'A', NameStartsWith: undefined};
        } else {
            updatedValue = {NameLessThan: undefined, NameStartsWith: newValue};
        }
    ...
    useEffect(() => {
        ...
        return () => { alphaPickerElement?.removeEventListener('alphavaluechanged', onAlphaPickerChange); };
    }, [onAlphaPickerChange]);
