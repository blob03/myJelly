...
class AboutTab {
    constructor(view, params) {
		...
		if (browser.tv || layoutManager.tv) {
			html += '<div> ' + globalize.translate('LabelRepositoryName') + ': <span style="font-weight:400;" class="aboutcontent">' + packageInfo.repository + '</span></div>';
			html += '<div> ' + globalize.translate('AppContact') + ' <span style="font-weight:400;" class="aboutcontent">' + packageInfo.contact + '</span></div>';
		} else {
			html += '<div> ' + globalize.translate('LabelRepositoryName') + ': <span><a is="emby-linkbutton" style="font-weight:400;" rel="noopener noreferrer" class="aboutcontent button-link emby-button" target="_blank" href="' + packageInfo.repository + '">' + packageInfo.repository + '</a></span></div>';
