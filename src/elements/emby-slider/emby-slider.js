...
/* eslint-disable indent */
    ...
    EmbySliderPrototype.attachedCallback = function () {
		...
		if (step)
			this.setKeyboardSteps(step, step);

		let bubble = this.hasAttribute("nobubble") ? false : true;
		if (bubble === false)
			this.disableBubble();
			
		if (layoutManager.tv) {
			this.classList.add('focusable');
            this.enableKeyboardDragging();
		}
    ...
    /**
     * Do not show the bubble.
     */
	EmbySliderPrototype.disableBubble = function () {
		const sliderBubbleTrack = this.parentNode.querySelector('.sliderBubbleTrack');
		if (sliderBubbleTrack) 
			sliderBubbleTrack.classList.add('hide');
	};
